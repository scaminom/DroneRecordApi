class ApplicationController < ActionController::API
  ...
  def no_route_found
    ...
    render_error_response(route_info, status: :not_found)
  end

  private

  def not_found(exception)
    render_error_response(format_error_message(exception), status: :not_found)
  end

  def unprocessable_entity(exception)
    render_error_response(format_error_message(exception), status: :unprocessable_entity)
  end

  def unauthorized_action(exception)
    render_error_response(format_error_message(exception), status: :forbidden)
  end

  def handle_parse_error
    render_error_response('Invalid JSON format', status: :bad_request)
  end

  def conflict_entity(exception)
    render_error_response(format_error_message(exception), status: :conflict)
  end
