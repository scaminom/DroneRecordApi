Rails.application.routes.draw do
  ...
  resources :estacion_carga_dcs, only: [:index] do
    collection do
      get 'filter_by_week(/:uav_id)', to: 'estacion_carga_dcs#index', defaults: { filter_type: :week }, as: :filter_by_week
      get 'filter_by_month(/:uav_id)', to: 'estacion_carga_dcs#index', defaults: { filter_type: :month }, as: :filter_by_month
      get 'filter_by_day(/:uav_id)', to: 'estacion_carga_dcs#index', defaults: { filter_type: :day }, as: :filter_by_day
      get 'current(/:uav_id)', to: 'estacion_carga_dcs#index', defaults: { filter_type: :current }, as: :current_estacion_carga_dcs
    end
  end

  resources :estacion_carga_acs, only: [:index] do
    collection do
      get 'filter_by_week(/:uav_id)', to: 'estacion_carga_acs#index', defaults: { filter_type: :week }, as: :filter_by_week
      get 'filter_by_month(/:uav_id)', to: 'estacion_carga_acs#index', defaults: { filter_type: :month }, as: :filter_by_month
      get 'filter_by_day(/:uav_id)', to: 'estacion_carga_acs#index', defaults: { filter_type: :day }, as: :filter_by_day
      get 'current(/:uav_id)', to: 'estacion_carga_acs#index', defaults: { filter_type: :current }, as: :current_estacion_carga_acs
    end
  end
